...
**Resolution:** `src/Cli/Command/AnalyseCommand.php` (search: `projectContextUnits`) now loads full current/base project context for enabled project rules in changed-only branch review mode, while `src/Rules/RuleRegistry.php` (search: `$projectUnits ?? $units`) keeps file-scoped rules on the narrowed unit list. The reported findings are still filtered back to changed files. `tests/Rule/RuleRegistryTest.php` (search: `testProjectRuleAccumulatorSeamObservesFullProjectContext`) locks the mechanism: an enabled project rule still receives the full project context even when the narrowed unit list is a single changed file.
