...
**Context:** Branch-review analysis can compare current findings against a Git base snapshot. On large repositories, materialising the full base tree before applying `--changed-only` turns a single-file review into full-repo archive and extraction work. The relevant implementation anchors are `src/Results/Review/GitArchiveSnapshot.php` (search: `public function create(string $projectRoot, string $ref, array $paths = [])`) and `src/Cli/Command/AnalyseCommand.php` (search: `baseSnapshotPaths`, search: `currentAnalysisPaths`).
...
**Verification:** `tests/Parser/PhpFileParserTest.php` (search: `testDeepScanBudgetProducesNonfatalRawTextUnit`) protects the parser seam, while `tests/Console/AnalyseCliDeepScanBudgetTest.php` (search: `testBoundedDeepScanKeepsTextLevelSecurityFindings`) and `testBoundedDeepScanDiagnosticIsVisibleInEveryFormat` protect retained security analysis and output visibility.
