...
**Context:** User-facing CLI commands (`analyse`, `summary`, `report`, `dashboard`, `init`) validate config and other inputs through helpers that throw on failure. A bare `throw new ConfigException("Config must include schemaVersion: ...")` gives the user a message but no remediation, and each catch site must guess what to print. Inconsistent catches across commands lead to: silent swallow (`src/Cli/Command/ReportCommand.php`, `src/Cli/Dashboard/DashboardStateFactory.php`), one-line `[CONFIG-ERROR]` prefix (`src/Cli/Command/SummaryCommand.php`), or structured `usageReport` (`src/Cli/Command/AnalyseCommandSetupBuilder.php`) — same throw site, three surfaces, none of them telling the user how to fix the underlying config. See `.goat-flow/learning-loop/lessons/workflow.md` "Lesson: Validator throws need an actionable hint, and the CLI catch must render it" (2026-05-27) for the reference incident.

**Approach:**

1. **Carry the hint on the exception object.** Extend `GruffPhp\Engine\Config\ConfigException` (file: `src/Engine/Config/ConfigException.php`) with an optional `string $hint = ''` constructor parameter and a `hint(): string` accessor. Default-empty preserves every existing throw site; new throw sites and the high-value retrofits supply the actionable step. Two reusable hint constants belong on the class:
