...
**Resolution:** M01 added `composer.json` (search: `"bin": [`), `bin/gruff` (search: `new Application()`), `src/Console/Application.php` (search: `final class Application`), `src/Command/AnalyseCommand.php` (search: `final class AnalyseCommand`), and `tests/Console/GruffCliTest.php` (search: `testVersionCommandRunsThroughBinary`).
