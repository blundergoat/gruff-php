...
- Diff and review: `GitDiffProvider` parses zero-context `git diff` output into changed files and inclusive changed-line ranges. `DiffFindingFilter` keeps line-located findings that touch changed ranges and keeps line-less findings only when their file changed. Branch review uses a Git archive snapshot of `--diff-vs` and compares stable finding identities instead of line-sensitive fingerprints.
