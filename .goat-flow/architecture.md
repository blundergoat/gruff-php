...
2. `Application` (Symfony Console subclass) registers the `analyse`, `report`, `dashboard`, and `list-rules` commands with version constant `0.1.0-dev`.
...
| Documentation | `docs.continue-comment`, `docs.missing-public-phpdoc`, `docs.missing-param-tag`, `docs.missing-return-tag`, `docs.missing-throws-tag`, `docs.return-comment`, `docs.stale-param-tag`, `docs.useless-phpdoc`, `docs.todo-density`, `docs.var-annotation-description`, `docs.missing-readme` | `docs.missing-public-phpdoc` requires local PHPDoc on every method declaration and reports errors. `docs.missing-return-tag` applies to every documented method/function except constructors/destructors. `docs.missing-readme` looks at `<projectRoot>/README.md` and is independent of the unit being analysed |
