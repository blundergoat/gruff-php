...
6. **the composite can be null, and so can a pillar or file grade** — `score.composite.{score,grade}` are `null` when the run evaluated nothing at all: an empty directory, or one whose every PHP file failed to parse, previously reported a perfect `100` and grade `A`. `score.topOffenders[].{score,grade}` are nullable for the same reason. `summary --format text` renders `Composite: n/a (nothing evaluated)`, `analyse --format markdown` renders `**Grade:** n/a (n/a/100)`, `analyse --format html` stamps `n/a`, and `analyse --format text` prints no score line at all.
