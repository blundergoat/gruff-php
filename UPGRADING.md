...
6. **the composite can be null, and so can a pillar or file grade** — `score.composite.{score,grade}` are `null` when the run evaluated nothing at all: an empty directory, or one whose every PHP file failed to parse, previously reported a perfect `100` and grade `A`. `score.topOffenders[].{score,grade}` are nullable for the same reason. Text and Markdown render `Composite: n/a (nothing evaluated)`, and HTML shows `not evaluated` in the grade stamp.
...
3. Regenerate the configuration if you hand-wrote one: `gruff-php init --force` rewrites it
