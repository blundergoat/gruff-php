...
/**
 * Holds the inline CSS and JavaScript that the standalone HTML inspection report embeds directly
 * into the page, so the single file a user opens in a browser is self-contained and needs no
 * network fetch to render or filter its findings.
 *
 * `HtmlReporter` calls in here while building the document produced by `gruff-php analyse
 * --format html`: `css()` returns the report's full visual styling, and `interactiveScript()`
 * returns the client-side finding filters, which are only wired in when the user opts into
 * `--report-interactive`. Everything here is static text with no per-run state.
 */
final readonly class HtmlReportAssets
{
    /**
     * Returns the client-side script that makes the report's finding filters live: reading the
     * severity, pillar, path, search, and group-by controls, re-rendering the matching findings, and
     * mirroring the active filters into the URL hash so a filtered view can be shared or reloaded.
     *
     * @return string - The filter script body wiring the controls to live filtering and URL-hash state; embedded verbatim inside the report's `<script type="module">` tag, and only when `--report-interactive` is set.
     */
...
JS;
    ...
    /**
     * Returns the report's stylesheet, starting from the base inspection-report theme and appending
     * only the rule blocks this run actually needs, so the inlined `<style>` never ships CSS for
     * sections the rendered page does not contain.
     *
     * @param bool $hasDiagnostics - True when the run produced diagnostics, so the diagnostics-panel rules are appended; false leaves them out.
     * @param bool $interactive - True when the user opted into `--report-interactive`, so the filter-form and grouped-finding rules are appended; false leaves them out.
     *
     * @return string - The full stylesheet for the standalone HTML report: the base rules plus whichever optional diagnostic and filter blocks the arguments requested.
     */
...
CSS;
        ...
        // The run produced diagnostics, so pull in the styling for the diagnostics panel the reader sees near the top of the report.
        if ($hasDiagnostics) {
            $css .= <<<'CSS'
...
CSS;
        ...
        // A plain, non-interactive report was requested, so return the styling accumulated so far (base plus any diagnostics rules) and skip the filter-UI rules entirely.
        if (!$interactive) {
            return $css;
        }

        // Interactive mode was requested, so append the filter-form and grouped-finding rules those controls need to render.
        return $css . <<<'CSS'
