...
{
    /**
     * @param array{project: string, paths: string, scanScope: string, failOn: string, config: string, baseline: string, noBaseline: string, noConfig: string, includeIgnored: string, reportInteractive: string} $state Dashboard form state to render.
     *
    ...
    public function dashboardHtml(array $state): string
    {
        ...
        // Hand back the full shell document; the iframe loads $scanUrl so the first scan starts without a user click.
    ...
    /**
     * @param string       $html        Report HTML generated by the scan.
     * @param string       $projectRoot Project root used by the scan.
     * @param list<string> $command     Command argv used for the scan.
     * @param int          $exitCode    Scan command exit code.
     * @param int          $durationMs  Scan duration in milliseconds.
     *
    ...
    public function injectDashboardMetadata(
        ...
        if (str_contains($html, '<body>')) {
            // Inject right after <body> so the meta script runs inside the document and can postMessage the parent.
            return str_replace('<body>', '<body>' . $metadata, $html);
        }

        // No <body> to anchor to (e.g. a bare error page), so prepend the meta script and still deliver it.
     ...
     *
     * @param string $message    Short error message to display.
     * @param string $detail     Detailed error output to include in the document.
     * @param int    $exitCode   Scan command exit code.
     * @param int    $durationMs Scan duration in milliseconds.
    ...
    {
        // Hand back a self-contained error document; message and detail are escaped so scan output cannot inject HTML.
     ...
     * @param string $fieldValue  Current value to pre-fill; reflected user input, so it is escaped here.
     * @param string $placeholder Hint text shown when the field is empty; defaults to none.
     ...
     *
     * @param string      $optionValue Value submitted when this option is chosen.
     * @param string      $selected    Currently selected value; this option is marked selected when it equals it.
     * @param string|null $label       Visible option text; falls back to $optionValue when null.
    ...
    {
        // Single-quoted heredoc keeps the CSS literal so $-prefixed values are not parsed as PHP variables.
        return <<<'CSS'
...
CSS;
    ...
    {
        // Single-quoted heredoc keeps the JS literal so its template strings and $ tokens are not parsed by PHP.
        return <<<'JS'
...
JS;
    ...
    {
        // Hand back a static placeholder document shown in the iframe until the first scan response replaces it.
    ...
    private function displayCommand(array $command): string
    {
        ...
        // Hand back a copy-pasteable command line; the original argv[0..1] become a stable "php bin/gruff-php" prefix.
     ...
     *
     * @param string $text Untrusted value, typically reflected form state, bound into HTML markup by callers.
