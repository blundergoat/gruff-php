...
final readonly class HtmlReporter
{
    ...
    private function pillars(AnalysisReport $report): string
    {
        ...
        if (!$hasMutationPillar) {
            $html .= $this->mutationPlaceholderCard();
        }

        return $html . '</div></section>';
    }

    private function mutationPlaceholderCard(): string
    {
        return '<div class="pillar pillar-empty">'
            ...
            . '</div>'
            . '<div class="empty-hint">Mutation data unavailable. Pass <code>--infection-report</code> to score this pillar.</div>'
            . '</div>';
    }
    ...
    private function distribution(AnalysisReport $report): string
    {
        ...
        $hasMutation = $report->mutation !== null;
        $gridClass = $hasMutation ? 'chart-grid' : 'chart-grid chart-grid-solo';
        $sectionAside = $hasMutation ? 'complexity and mutation' : 'cyclomatic complexity';
        ...
        $right = $hasMutation
            ? '<div class="chart-card"><div class="title">mutation score · per file</div>' . $this->mutationList($report) . '</div>'
            : '';

        return sprintf(
            '<section class="chart-section"><div class="section-head">distribution <span class="aside">%s</span></div><div class="%s">%s%s</div></section>',
            $this->escape($sectionAside),
            $this->escape($gridClass),
            $histogram,
            $right,
        );
    }
