...
- **Regex purpose comments follow their owning statement** - `docs.regex-comment` accepts own-line explanations above a regex's nearest statement, including multiline ones, or across contiguous runs whose statements each own a configured call; blank lines, unrelated code, branches, callable boundaries, and same-line code break coverage. It recognises only explicit `preg_replace('/\s+/', ' ', ...)` whitespace-fold contracts and resolves named arguments by parameter. Broad regex/pattern docblocks cover one call; generic or misplaced comments still report. Retained messages stay unchanged, but `gruff.baseline.v2` groups may change; review the delta before updating accepted debt.
- **Named callback boundaries stay named** - `waste.one-line-method` preserves exact same-class methods referenced by PHP first-class callables or vetted two-item callable arrays. Numeric keys follow runtime slots regardless of order; associative arrays and unproven dynamic, foreign, computed, string, or inheritance shapes still report. Allow intentional runtime wiring with `rules.waste.one-line-method.options.allowedSymbols`.
- **Boolean state names use conservative word boundaries** - `naming.boolean-prefix` accepts whole-token `requested`, `present`, `enabled`, and `allowed` suffixes plus contextual subject-first `requires`; typed properties and parameters also accept exact `resolved`, `limited`, and `printable` names. Fragments, vague names, and incomplete propositions still report. Configure vocabularies with `stateSuffixAllowlist`/`propositionVerbAllowlist`, exact exceptions with `acceptedBooleanNames`, or private/local-only analysis with `includePublicApi: false`.
- **Constructor advice and universal abbreviations are quieter** - `modernisation.named-argument-opportunity` excludes constructors but retains configurable function/method/static-call advice; `naming.abbreviation-allowlist` adds `dto`/`utc` defaults while `dob` stays project-specific and four-character `uuid` remains out of band.
- **Machine-readable findings distinguish direct fixes from compatibility decisions** - JSON, hook, and SARIF mark `docs.regex-comment`, `docs.missing-constant-phpdoc`, and `waste.one-line-method` as `APPLY`; named-argument, abbreviation, and caller-visible Boolean advice as `CONSIDER`; and private property/callable Boolean names as `APPLY`. Hatches carry `configurationKey`; `CONFIGURE` remains for deterministic configuration-only fixes. Identity-neutral metadata leaves `gruff.analysis.v2`, scoring, and text/Markdown unchanged.
- **Pattern-family constant comments stay bounded** - `docs.missing-constant-phpdoc` covers at most five contiguous names under explicit `patterns` or `regexes` comments, including multi-name declarations; later names and structural boundaries report. Overflow findings describe the cap and expose `commentQuality: bounded-group-overflow`, comment kind, and limit without changing coverage. Messages affect baseline and stable identities, so review or regenerate accepted rows. `keys`, `values`, and mixed comments remain uncapped.
