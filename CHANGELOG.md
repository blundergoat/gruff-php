...
0.5.1 removes the documented false-positive shapes while retaining their
counterexamples: multiline regex ownership, exact named callback boundaries,
clear Boolean state/proposition names, and short bounded pattern families gain
conservative matching, while abbreviation and named-argument advisories gain
machine-readable decision context. Rule messages, severities, thresholds,
scoring, identities, `gruff.analysis.v2`, and `--fail-on` behaviour are
unchanged.

- **Regex purpose comments follow their owning statement** - `docs.regex-comment` now accepts an own-line explanation directly above a multiline condition, assignment, return, or other nearest statement. One comment can also cover a physically contiguous sibling run when every statement owns a configured regex call; blank lines, new comments, unrelated statements, branches, and callable boundaries stop the group. A plain-language whitespace-fold contract is accepted only for a statically visible `preg_replace('/\s+/', ' ', ...)` transformation. Named arguments are resolved by PHP parameter name, so a valid reordered transformation stays quiet while swapped values still report. Previous-statement, trailing same-line, generic `safe`/`valid`, and unrelated replacement cases also report. A broad regex/pattern callable docblock now covers at most one configured call, so callables containing several operations may gain findings until each operation has local purpose documentation. This tightening is a deliberate behaviour change: retained finding messages are unchanged, but a `gruff.baseline.v2` file can show removed false-positive groups as absent and newly re-flagged broad-docblock calls as new; review that delta before updating accepted debt.
- **Named callback boundaries stay named** - `waste.one-line-method` now preserves an exact same-class comparator, serializer, or other method referenced with PHP first-class callable syntax or a vetted two-element callable array using `$this`, `self::class`, `static::class`, `__CLASS__`, or the fully resolved declaring class. Explicit numeric keys follow PHP's runtime slots even when keys `1` and `0` appear in reverse source order; associative metadata with callback-like values remains visible to both wrapper and dead-code rules. Ordinary one-shot wrappers and unproven dynamic, foreign, computed, string, or child-name-to-parent-declaration callback shapes still report; use `rules.waste.one-line-method.options.allowedSymbols` when runtime/framework wiring makes one of those conservative shapes intentional.
- **Boolean state names use conservative word boundaries** - `naming.boolean-prefix` now accepts `requested`, `present`, `enabled`, and `allowed` only as whole-token suffixes on multi-token names, plus subject-first `requires` propositions that have context on both sides. Typed properties and parameters also accept the whole names `resolved`, `limited`, and `printable`; single-token callables, vague nouns, incomplete propositions, and suffix fragments still report. Projects can replace the shipped suffix and proposition vocabularies through `stateSuffixAllowlist` and `propositionVerbAllowlist`, while `acceptedBooleanNames` remains the exact non-breaking hatch. `includePublicApi: false` provides a private/local-only mode for libraries that must not pressure caller-visible renames.
- **Constructor advice and universal abbreviations are quieter** - `modernisation.named-argument-opportunity` explicitly keeps direct constructor calls outside its advisory scope while preserving function, method, and static-call findings plus the configurable `minPositionalArguments` floor. `naming.abbreviation-allowlist` now treats `dto` and `utc` as universal defaults; `dob` remains project vocabulary and four-character `uuid` remains outside the default length band.
- **Machine-readable findings distinguish direct fixes from compatibility decisions** - JSON, hook, and SARIF metadata now classify retained `docs.regex-comment`, `docs.missing-constant-phpdoc`, and `waste.one-line-method` findings as `APPLY`; named-argument and abbreviation advice as `CONSIDER`; and Boolean naming findings as `APPLY` for private/local declarations or `CONSIDER` for caller-visible declarations. Intentional configuration hatches expose their full `configurationKey`; `CONFIGURE` is reserved for a deterministic configuration-only fix and is not emitted unconditionally. Finding messages, identities, `gruff.analysis.v2`, scoring, and the family-frozen text/Markdown finding blocks are unchanged.
- **Pattern-family constant comments stay bounded** - `docs.missing-constant-phpdoc` now recognises explicit `patterns` and `regexes` comments as documentation for at most five immediately contiguous constant names, counting every name in multi-name declarations. The sixth and later names, visibility changes, blank/non-constant/PHPDoc/local-comment boundaries, comments trailing a previous declaration, and unrelated or unexplained constants still report. Existing group words such as `keys` and `values` remain uncapped, and a mixed comment follows that shipped behavior.
