...
- **`report` rejects unknown rule filters as a usage error** - `report --include-rule`/`--exclude-rule` now validate ids up front: a mistyped id fails fast (exit code 2) before the init prompt, any config write, or the child `analyse` run, matching how `analyse` already validates the same options.
- **Internal namespace consolidation** - `src/` is reorganised from 19 top-level directories into six package areas (`Cli`, `Engine`, `Rules`, `Results`, `Output`, `Support`). The Composer PSR-4 root stays `GruffPhp\ => src/`, and rule IDs, CLI command names, config keys, JSON report schemas, and baseline identities are unchanged, so CLI, hook, and CI output stay identical. Code that imported gruff-php's internal classes directly must update to the new namespaces (for example a `GruffPhp\Rule\` import becomes `GruffPhp\Rules\`).
